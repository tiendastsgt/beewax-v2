...
import { AuthRequest, Alert, ApiResponse } from '../types';
...
router.get('/', (req: AuthRequest, res: Response<ApiResponse<Alert[]>>) => {
...
router.get('/:id', (req: AuthRequest, res: Response<ApiResponse<Alert>>) => {
...
router.post('/', (req: AuthRequest, res: Response<ApiResponse<Alert>>) => {
...
router.put('/:id/acknowledge', (req: AuthRequest, res: Response<ApiResponse<Alert>>) => {
...
router.put('/:id/resolve', (req: AuthRequest, res: Response<ApiResponse<Alert>>) => {
  const alertIndex = alerts.findIndex(a => a.id === req.params.id);
  if (alertIndex === -1) {
    return res.status(404).json({ success: false, error: 'Alert not found' });
  }
