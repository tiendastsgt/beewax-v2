...
import { AuthRequest, Firmware, OTAApplication, ApiResponse } from '../types';
...
let otaApplications: OTAApplication[] = [];

// GET /api/ota/firmwares - Get all firmwares
router.get('/firmwares', (req: AuthRequest, res: Response<ApiResponse<Firmware[]>>) => {
  res.json({ success: true, data: firmwares });
});

// GET /api/ota/firmwares/:id - Get firmware by ID
router.get('/firmwares/:id', (req: AuthRequest, res: Response<ApiResponse<Firmware>>) => {
...
router.post('/firmwares', (req: AuthRequest, res: Response<ApiResponse<Firmware>>) => {
...
router.put('/firmwares/:id/activate', (req: AuthRequest, res: Response<ApiResponse<Firmware>>) => {
...
router.post('/applications', (req: AuthRequest, res: Response<ApiResponse<OTAApplication>>) => {
  ...
  const newApplication: OTAApplication = {
    id: Date.now().toString(),
    hiveId,
    firmwareId,
    status: 'pending',
...
router.get('/applications/:id', (req: AuthRequest, res: Response<ApiResponse<OTAApplication>>) => {
...
router.get('/applications', (req: AuthRequest, res: Response<ApiResponse<OTAApplication[]>>) => {
  const { hiveId, status } = req.query as any;
